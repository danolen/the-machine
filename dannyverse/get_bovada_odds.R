...
get_bovada_odds <- function(sport) {
  ...
  bovada_odds <- fromJSON(url) %>%
    ...
    select(gamedate, AwayTeam, HomeTeam,
    ...
    select(gamedate, AwayTeam, HomeTeam,
           # AwayStartingPitcher, HomeStartingPitcher, 
           bet_type, type, Odds, any_of("SpreadTotal")) %>%
    ...
    melt(id.vars = c("gamedate", "AwayTeam", "HomeTeam", 
                     # "AwayStartingPitcher", "HomeStartingPitcher",
                     "bet_type", "type")) %>%
    mutate(name = paste0(type, ".", variable)) %>%
    select(-type, -variable) %>%
    mutate(value = as.numeric(value)) %>%
    dcast(gamedate + AwayTeam + HomeTeam + 
            # AwayStartingPitcher + HomeStartingPitcher + 
            bet_type ~ name, fun.aggregate = mean) %>%
